...
public class AsyncScheduler {
    ...
    public SchedulerTask run(Consumer<SchedulerTask> runnable) {
        SchedulerTask task = new SchedulerTask(runnable, 1, 0, 0, TASKS_ID, true);
    ...
    public SchedulerTask runLater(Consumer<SchedulerTask> runnable, long later) {
        SchedulerTask task = new SchedulerTask(runnable, 1, later, 0, TASKS_ID, true);
        tasks.put(TASKS_ID, task);
        TASKS_ID++;
        return task;
    }
    ...
    public SchedulerTask runTimer(Consumer<SchedulerTask> runnable, long later, long period) {
        SchedulerTask task = new SchedulerTask(runnable, Integer.MAX_VALUE, later, period, TASKS_ID, true);
        tasks.put(TASKS_ID, task);
        TASKS_ID++;
        return task;
    }
    ...
    public SchedulerTask runTimer(Consumer<SchedulerTask> runnable, long later, long period, long limit) {
        SchedulerTask task = new SchedulerTask(runnable, limit, later, period, TASKS_ID, true);
        tasks.put(TASKS_ID, task);
        TASKS_ID++;
        return task;
    }
